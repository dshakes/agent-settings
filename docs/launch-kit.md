...
### See it in action (the maintainer explicitly rewards "show me before I run it" — link these)
- The README hero already renders on GitHub: the **animated explainer** + the **self-fixing loop diagram** (`assets/sdlc-loop.svg`) + a ~25s **terminal demo** (`demo/preview.gif`).
- **The clincher — record it (§4 below):** a ~20s capture of the real loop on a PR (review flags **Blocking** → Builder pushes a fix → checks **green**). Embed it as the README hero and paste the link in the issue. This single asset is the highest-leverage thing for approval; everything else is already in place.
